...
Рядом с исходниками лежат `README.docx` и `DATA_MAP.docx` — для тех, кому удобнее Word
(таблицы, оглавление, печать). Пересобрать после правки `.md`:

```bash
pip install markdown pywin32        # один раз
python tools/md2docx.py tools/README.md tools/DATA_MAP.md
```

Конвертер `md2docx.py` собирает HTML и сохраняет через установленный Microsoft Word
(нужен Windows с Word). Исходник — всегда `.md`; `.docx` пересобирается из него.

## `DATA_MAP.md` — что импортируется из 1С и где журнал

Справочник «что откуда взялось»: цепочка «объект 1С → процедура → таблицы», основные
поля всех наполняемых таблиц Firebird, метки времени (какими полями вообще можно
датировать импорт), описание коллекций MongoDB как **журнала** — с упором на
`res.insert_at` (когда запись добавлена) и `res.ref_id` (какой ID присвоен в HiTek).

Отдельно — две ловушки сопоставления: Mongo хранит UTC, а Firebird местное время
(разница ровно 5 часов), и в журнале есть дубли по одному `ref_id`.

## `postman/` — наборы запросов для Postman

Два самостоятельных файла, переносятся на любую машину импортом:

| Файл | Что внутри |
|---|---|
| `postman/C1CServ.postman_collection.json` | 24 запроса: служебные, импорт из 1С по всем 10 ручкам, поиск по ERP-id, обратное создание в 1С, выгрузка `/exp2/v1/*` |
| `postman/fb-port.postman_collection.json` | 14 запросов: метаданные (`ProcList`, `Proc`, …) и вызовы процедур через `POST /query`, включая `EXP2_*` |

**Перенос на другую машину:** скопировать файл → в Postman *Import* → выбрать файл.
Отдельный файл окружения не нужен: адрес задаётся переменной `baseUrl` внутри самой
коллекции (вкладка **Variables**), там же `guid`, `refId`, `proc`.

```
baseUrl по умолчанию   C1CServ  http://127.0.0.1:3738     fb-port  http://127.0.0.1:3333
тестовый стенд         C1CServ  http://192.168.7.143:3738
исторически (Postman)  C1CServ  http://192.168.10.65:3738  fb-port  http://localhost:8080
```

Наборы собраны из двух источников: запросы, найденные в локальном Postman
(`C1_Nomenklature`, `C1_upd_nom_by_nom_id`, `C1_GUID`, `/query`, `/Proc` — помечены в
описаниях), плюс остальные ручки, выверенные по коду. Проверены на стенде.

Две особенности, из-за которых запросы легко составить неверно:

- **C1CServ отдаёт ошибки с кодом HTTP 201** — смотреть надо тело, поле `err.errCode`
  (10, 20, 30, 40), а не статус ответа.
- **В fb-port поле называется `transactonType`** (опечатка в исходном API), и если его
  не указать — транзакция будет `READ_WRITE`, то есть на запись. Для чтения указывайте
  `READ_ONLY` явно.
