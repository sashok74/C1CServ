...
## 13а. Как определить топологию обмена на любом хосте

Где заданы адреса и порты (файлы конфигурации, переменные, хардкод с указанием
`файл:строка`), как выяснить это на живом хосте и как восстановить картину постфактум —
отдельный документ **`TOPOLOGY_HOWTO.md`**. Автоматизированное обследование хоста —
`tools/discover-stack.sh` (см. `tools/README.md`).
