...
- Firebird `:3050` и fb-port `:3333` доступны с рабочей станции. Про веб-сервис 1С `192.168.10.58:80` так сказать **нельзя**: проверено 2026-08-13, что в этой подсети «отвечает» любой адрес и любой порт (включая заведомо несуществующие) — отвечает посредник на маршруте, поэтому проверка портом и `ping` там недостоверна, см. §13а.
...
## 13а. Как вручную найти адреса и порты

Автоматически всё это собирает `tools/discover-stack.sh`
(`ssh <host> 'sudo bash -s' < tools/discover-stack.sh`), полная версия инструкции —
`TOPOLOGY_HOWTO.md`. Ниже — ручной способ, когда скрипта под рукой нет.

### Где смотреть в файлах

| Что нужно | Где задано |
|---|---|
| Порт и интерфейс C1CServ | `.env` → `SERVER`, `PORT` (умолчание в `src/server.ts` — `127.0.0.1:3737`) |
| **Адрес 1С** | `.env` → `C1_WEBSERVER`; пути сервисов — в коде, `src/types/ExportSchemes.ts`, поле `servC1Path` |
| Адрес fb-port, который зовёт C1CServ | `.env` → `DB_HOST`, `DB_PORT` |
| Адрес MongoDB | `.env` → `MONGODB_SERVER`/`MONGODB_USER`/`MONGODB_PASSWORD`/`MONGODB_BASE` (порт 27017 захардкожен в `src/modules/db.ts:10`) |
| Порт fb-port | его `.env` → `SERVER`, `PORT` (умолчание `127.0.0.1:3333`) |
| **Адрес и имя базы Firebird** | `.env` fb-port → `DB_HOST`, `DB_PORT`, `DB_NAME`; строка соединения собирается в `src/modules/db.ts:24` |
| Расшифровка алиаса базы | на сервере БД: `/opt/firebird/databases.conf` или `/etc/firebird/*/databases.conf` |

Файл `.env` лежит в корне проекта: `/opt/c1cserv/.env`, `/root/node-app/C1CServ/.env`,
`/root/node/fb-port/.env`. При systemd-развёртывании его заменяет `EnvironmentFile=`
(например `/etc/c1-test/c1cserv.env`). Полный список переменных C1CServ —
`src/types/environment.d.ts`.

### Как найти на работающем хосте

```bash
# 1. кто слушает интересующие порты
ss -tlnp | grep -E ':(3737|3738|3333|27017|3050)'

# 2. по PID — каталог сервиса и переменные, с которыми он ЗАПУЩЕН (нужен root)
ls -l /proc/<PID>/cwd
tr '\0' '\n' < /proc/<PID>/environ | grep -E '^(PORT|SERVER|DB_|MONGODB_|C1_)'

# 3. как запускается
systemctl cat <юнит> | grep -E 'ExecStart|EnvironmentFile|WorkingDirectory'
pm2 list                       # в проде fb-port работает под PM2, а не systemd
```

`/proc/<PID>/environ` важнее файла: **`dotenv` не перекрывает уже заданные переменные**,
поэтому при запуске через systemd лежащий рядом `.env` может не действовать вовсе.

### Как проверить, что адрес рабочий

```bash
curl -s http://127.0.0.1:3738/test_db | grep -o '"DB_NAME":"[^"]*"'   # реальная база C1CServ
curl -s http://127.0.0.1:3333/ProcList | head -c 200                   # живость fb-port
curl -s "http://<адрес-1С>/unf/hs/ht/get_measure/<GUID из c1_data>"     # живость 1С
```

⚠️ **Проверка `ping` и «открытым портом» может врать.** В сети с прозрачным прокси или
policy-routing соединение «устанавливается» с любым адресом и портом. Контроль: пробейте
заведомо несуществующий адрес той же подсети — если он тоже «отвечает», результаты
бессмысленны. Достоверен только осмысленный прикладной ответ (команды выше).

### Чего в следах нет

Адрес 1С не хранится **нигде**, кроме `.env`: в журнале MongoDB лежат только сами ответы
и время, C1CServ пишет в лог `axios.get: <путь>` без хоста, в базе адреса нет. Поскольку
`.env` в `.gitignore`, в бэкапе исходников его тоже нет — исторический адрес можно взять
только с машины, где сервис работал. Строка соединения с Firebird, наоборот, печатается
fb-port при каждом старте (`src/modules/db.ts:25`) и обычно есть в journald или
`~/.pm2/logs/`.
